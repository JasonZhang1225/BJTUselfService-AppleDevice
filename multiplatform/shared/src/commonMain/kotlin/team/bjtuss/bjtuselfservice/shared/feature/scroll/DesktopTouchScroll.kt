 ...
 * 兼容层不会给 AWT 发送滚轮事件。这里仅在 Windows 桌面端为已有 ScrollableState 增加
 * 一层越过 touch-slop 后才生效的纵向拖动映射；普通点击、按钮和长按仍保留给
 * 子组件。macOS、Android、iOS 使用平台原生触摸滚动，不叠加这层。
 */
@Composable
fun Modifier.desktopTouchScroll(
    ...
    // 平台门禁必须在兼容层内部，调用方即使误传 enabled=true 也不能污染 Android/iOS/macOS。
    if (!shouldEnableDesktopTouchScroll(currentPlatform(), enabled)) return this
...
internal fun shouldEnableDesktopTouchScroll(
    platform: PlatformInfo,
    requested: Boolean,
): Boolean = requested && platform.isWindows
