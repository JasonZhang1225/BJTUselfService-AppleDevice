...
> 阶段状态：**1.7.3-KMP-A 已发布。** 系统级 MSI / 前台 UAC / 卸载清凭据。下一步为 M13。
> 分支创建点：`9d8da18`；上游对照基线：`v1.7.0@419313d`；KMP 自身基线：**`v1.7.3-KMP-A`（git tag，`6c69c85`）**；上一发布 `v1.7.3-KMP@84bf479` 保留。
...
- **2026-08-16 发布 `1.7.3-KMP-A`**：git tag `v1.7.3-KMP-A`（`6c69c85`）。Windows MSI/EXE 本机构建上传。Android/iOS/macOS 本机无 SDK 未打。发布页：https://github.com/JasonZhang1225/BJTUselfService-KMP-Refreshed/releases/tag/v1.7.3-KMP-A
...
- **Windows 卸载清凭据待复测**：请装 Release 上的 `BJTUselfServiceKMP-1.7.3-KMP-A.msi` 后再卸，确认 AppData 缓存和注册表凭据被删。
...
1. **用户安装 1.7.3-KMP-A MSI 后再卸**：确认卸载后不再记住密码、AppData 缓存消失。
