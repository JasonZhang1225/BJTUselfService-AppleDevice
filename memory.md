...
- 2026-08-03 登录凭据 UI 已收尾：macOS 账号/密码改为真实 AppKit `NSTextField`/`NSSecureTextField`，限制 ASCII/Roman 输入源，统一蓝色焦点、高度与 14pt 圆角；保留系统中文右键菜单“自动填充→密码…”，已回退会覆盖它且无响应的自定义英文 `Passwords…` 菜单。学号 Return/Tab 转密码、密码 Return 提交，并对密码提供器的批量末尾插入执行整段替换；`ComposeWindow.windowHandle` 挂载消除了旧 JNA/AWT 句柄转圈互锁。iOS 原生输入容器已透明化，暗色账号/密码框和验证码框统一为 `#23262C`、14pt 圆角与相同描边，三端回归构建通过；iOS 暗色修复仍待 Simulator/真机最终目视。`cas.bjtu.edu.cn` AASA 为 404、`mis.bjtu.edu.cn` 为 403，故网页系统凭据不保证跨字段成组返回，应用自身 Keychain 凭据仍成组恢复。
...
- **登录页仍缺 iPhone 真机验收**：iOS Simulator 的软键盘收起、Next/Go 与白色键盘 spacer 修复已通过；本轮暗色原生容器透明与三类输入框统一已编译、尚待 Simulator/真机目视，真机还需核对 Password AutoFill 一次成组填充与 Keychain 保存/重启读取。macOS 系统中文右键菜单与“自动填充→密码…”已通过可见验证；跨页 token 仍未全面收敛。
...
1. **M5.5 登录页与凭据收尾**：在 iPhone 真机专项验证 Keychain 保存、重启读取和退出清理，并目视密码键盘/自动填充；macOS 待用户用真实系统密码条目复验一次选择填充，不在调试记录中暴露凭据。
