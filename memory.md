...
> 最后更新：2026-08-31
> 当前分支：`main`；显示/打包版本 **`1.7.5-KMP`**。邮箱空详情页修复已提交并推送；Windows 本机无法打 IPA，由用户在 Mac 打包。Android debug 装到 `25091RP04C`（`6c5a737e`）。未改 `windowsApp/`、`desktopApp/` 平台工程。
...
- **2026-08-31 邮箱空详情页**：快速打开/关闭到第 3～4 封时，上一封原生详情的 `onDispose` 会清掉下一封的 loading，且详情 HTTP 卡在同一把列表互斥锁里。已去掉「选择一封邮件查看内容」全屏占位（宽屏未选中只留空白阅读区）；`clearSelectedMessage` 按打开代次忽略迟到 dispose；详情读取不再占用列表锁。`:shared:desktopTest` 邮箱模型与导航测试通过。Android/iOS 实机快速开合尚未复测。
...
- **M15 邮箱功能边界**：只读文件夹/列表/详情、分页加载与 Apple Mail 方向的响应式 UI 已完成首轮；紧凑端邮箱主页、邮件详情和写信/回复统一走平台原生页面层级。快速开合邮件空详情页已在模型层修掉，待 Android/iOS 实机复测。Android 启动不再预热 WebView。本地正文缓存、转发、删除、移动、附件上传/下载和真实发送仍待单独切片。

## 3. 接下来 1～3 个阶段

1. **Android/iOS 复测邮箱快速开合**：确认原生详情不再出现「选择一封邮件」空页，再发 `1.7.5-KMP`。
2. **发布 `1.7.5-KMP`**：用户在 Mac/iOS 做最终验收后打 tag、建 GitHub Release 并上传四端产物。本轮未提交、未打 tag。
