...
> 阶段状态：**1.7.3-KMP 已发布；Windows 安装器 per-user 修复已提交、推送并覆盖 Release 资产。** 下一步为 M13。
...
- **2026-08-16 Windows 安装器 per-user 修复**（未打 tag，随 main 推送）：`windowsApp` 加 `perUserInstall = true`，改按用户安装到 `%LOCALAPPDATA%\Programs`，**消除未签名安装器的 UAC 提权弹窗**（原系统级安装在后台闪烁、易被误认卡死）；jpackage 参数已确认含 `--win-per-user-install`。新版 `交大自由行 KMP-1.7.3.exe`（109M）已上传覆盖 Release 资产（旧 113,579,520 字节版）。用户确认了免 UAC 方案，但未实测安装；安装向导品牌化（横幅/右上角图）jpackage 不支持，如需完全定制需评估 Inno Setup。
...
- **Windows 安装器未实测**：per-user 版 EXE 已上传但用户尚未在 Windows 上卸载旧版并安装验证。
...
1. **用户验证 Windows per-user 安装器**（先卸载旧版 `BJTUselfServiceKMP-1.7.3` 再装新版；确认无 UAC、安装路径为 `%LOCALAPPDATA%\Programs\交大自由行 KMP`）。
