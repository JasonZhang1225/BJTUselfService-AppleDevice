...
> 阶段状态：**1.7.3-KMP-A 发布中。** 撤回 perUserInstall；系统级 MSI / 前台 UAC / 卸载清凭据。下一步为 M13。
> 分支创建点：`9d8da18`；上游对照基线：`v1.7.0@419313d`；KMP 自身基线：**`v1.7.3-KMP-A`（待打 tag）**；上一发布 `v1.7.3-KMP@84bf479` 保留。
